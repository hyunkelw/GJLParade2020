    ...
    public class CameraBaseControl
    {
        ...
        public void UpdateCameraPosition()
        {
            if (firstPerson)
            {
                ...
                cam.position = player.position + Direction.WorldToLocalDirection(cameraOffset, player.rotation); // Vector3.Slerp(cam.position, player.position + Direction.WorldToLocalDirection(cameraOffset, player.rotation), Time.deltaTime * smoothPosition);
            }
            else
            {
